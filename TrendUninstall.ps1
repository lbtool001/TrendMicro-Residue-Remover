$winVer = $PSVersionTable.PSVersion
$psExe = if (Get-Command pwsh -ErrorAction SilentlyContinue) { "pwsh" } else { "powershell.exe" }

if (-not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) {
    Start-Process $psExe -ArgumentList "-NoProfile -ExecutionPolicy Bypass -Command `"& {`$url='$url'; `$destDir='$destDir'; `$zipPath='$zipPath'; `$(Invoke-RestMethod https://raw.githubusercontent.com/lbtool001/TrendMicro-Residue-Remover/refs/heads/main/TrendUninstall.ps1 | Invoke-Expression)}`"" -Verb RunAs
    return
}
...
try{
    ...
    if(Test-Path "C:\Temp"){
        ...
        if($choice.Trim().ToUpper() -eq "Y"){
            $deleted=$false
            for($i=0;$i -lt 5;$i++){
                try{Remove-Item -Path "C:\Temp" -Recurse -Force;$deleted=$true;break}catch{Start-Sleep -Milliseconds 500}
            }
